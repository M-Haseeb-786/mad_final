...
const Details = ({ navigation, route }) => {
    ...
    useEffect(() => {
        ...
        let obj = [];
        chapters.map((chapter, index) => index !== 0 && obj.push({ title: chapter[0], data: [chapter[1]] }))
        setChaps(obj)
        setData(chapters)
    }, [])
    ...
    const handleSearch = () => {
        ...
        let temp = chaps.filter( ch => ch.title.indexOf(search) !== -1 || ch.data.join("").indexOf(search) !== -1)
    ...
    return (
        <View>
            <TextInput 
                ...
                onChangeText={text => {setSearch(text); setError("")}}
